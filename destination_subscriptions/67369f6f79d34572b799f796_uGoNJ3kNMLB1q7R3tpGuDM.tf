...
resource "segment_destination_subscription" "id-67369f6f79d34572b799f796_uGoNJ3kNMLB1q7R3tpGuDM" {
  ...
  settings = jsonencode({
    ...
    properties = {
      list_price = {
        "@template" = "{{__segment_entities.cortega-dev.listings.LIST_PRICE}}"
      }
      list_status = {
        "@template" = "{{__segment_entities.cortega-dev.listings.LIST_STATUS}}"
      }
      product_name = {
        "@template" = "{{__segment_entities.cortega-dev.products.PRODUCT_NAME}}"
      }
      product_rank = {
        "@template" = "{{__segment_entities.cortega-dev.products.PRODUCT_RANK}}"
      }
    }
    time = {
      "@path" = "$.receivedAt"
